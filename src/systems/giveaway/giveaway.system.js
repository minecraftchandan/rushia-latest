...
class GiveawaySystem {
  ...
  async loadConfig() {
    try {
      const configPath = path.join(__dirname, '../../data/gw.json');
  ...
  async ensureConfig() {
    if (!this.gwConfig) await this.loadConfig();
  }

  async isAllowedGuild(guildId) {
  ...
  async trackTask(guildId, userId, username, taskType, client) {
    try {
      if (!await this.isAllowedGuild(guildId)) {
        return null;
      }
