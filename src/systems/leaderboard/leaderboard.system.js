...
async function handleRlbCommand(message) {
  ...
  try {
    ...
    const userDropCount = allDroppers.find(u => u.userId === message.author.id)?.drop_count ?? 0;
    ...
    embed.setFooter({ text: `Page 1/${totalPages} | Participants: ${allDroppers.length} | Total Drops: ${totalDrops} | Your Drops: ${userDropCount}` });
...
async function handleBackButton(interaction) {
  try {
    ...
    const totalDrops = allDroppers.reduce((sum, user) => sum + user.drop_count, 0);
    const totalPages = Math.ceil(allDroppers.length / perPage);
    const userDropCount = allDroppers.find(u => u.userId === interaction.user.id)?.drop_count ?? 0;
    ...
    embed.addFields({ name: '\u200b', value: rankings });
    embed.setFooter({ text: `Page ${page + 1}/${totalPages} | Participants: ${allDroppers.length} | Total Drops: ${totalDrops} | Your Drops: ${userDropCount}` });
...
function buildClashEmbed(interaction, allClash, page, userId, userClashCount) {
  const perPage = 10;
  ...
  if (!slice.length) {
    ...
    embed.addFields({ name: '\u200b', value: rankings });
    embed.setFooter({ text: `Page ${page + 1}/${totalPages} | Participants: ${allClash.length} | Total Clashes: ${totalClashes} | Your Clashes: ${userClashCount ?? 0}` });
  }

  return embed;
}

function buildClashComponents(userId, allClash, page) {
  const perPage = 10;
  const back = new ButtonBuilder()
    .setCustomId(`back_to_drops_${userId}_0`)
    .setLabel('Back')
    .setStyle(ButtonStyle.Secondary)
    .setEmoji('⬅️');

  const prevButton = new ButtonBuilder()
    .setCustomId(`clash_prev_${userId}_${page}`)
    .setLabel('◀')
    .setStyle(ButtonStyle.Secondary)
    .setDisabled(page === 0);

  const nextButton = new ButtonBuilder()
    .setCustomId(`clash_next_${userId}_${page}`)
    .setLabel('▶')
    .setStyle(ButtonStyle.Secondary)
...
async function handleClashButton(interaction) {
  try {
    ...
    const userClash = allClash.find(u => u.userId === interaction.user.id);
    const embed = buildClashEmbed(interaction, allClash, 0, interaction.user.id, userClash?.clash_count ?? 0);
    const row = buildClashComponents(interaction.user.id, allClash, 0);
    await interaction.update({ embeds: [embed], components: [row] });
  } catch (error) {
...
async function handleClashPagination(interaction) {
  ...
  try {
    ...
    const userClash = allClash.find(u => u.userId === interaction.user.id);
    const embed = buildClashEmbed(interaction, allClash, newPage, interaction.user.id, userClash?.clash_count ?? 0);
    const row = buildClashComponents(interaction.user.id, allClash, newPage);
    await interaction.update({ embeds: [embed], components: [row] });
    return true;
  } catch (error) {
...
async function handleRlbPagination(interaction) {
  ...
  embed.addFields({ name: '\u200b', value: rankings });
  embed.setFooter({ text: `Page ${newPage + 1}/${totalPages} | Participants: ${allDroppers.length} | Total Drops: ${totalDrops} | Your Drops: ${userDropCount}` });
